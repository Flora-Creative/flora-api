...
connectSMTPPort hostname port =
  (handleToStream <$> connectTo hostname port) >>= connectStream
...
tryCommand conn cmd tries expectedReply = do
  ...
  case () of
    ...
    _
      | tries > 1 -> tryCommand conn cmd (tries - 1) expectedReply
      | otherwise -> do
...
sendCommand (SMTPC conn _) meth = do
 ...
 where
  command = case meth of
    ...
    (HELP msg  ) -> if null msg then "HELP\r\n" else "HELP " ++ msg
    NOOP         -> "NOOP"
    RSET         -> "RSET"
    QUIT         -> "QUIT"
    (DATA _) ->
...
doSMTPPort host port = bracket (connectSMTPPort host port) closeSMTP
...
sendMimeMail to from subject plainBody htmlBody attachments con = do
  ...
  renderedMail <- renderMail' myMail
  sendMail from [to] (lazyToStrict renderedMail) con
  where address = Address Nothing . T.pack
...
sendMimeMail'
  ...
  -> IO ()
sendMimeMail' to from subject plainBody htmlBody attachments con = do
  let myMail = simpleMailInMemory (address to)
                                  (address from)
                                  (T.pack subject)
                                  plainBody
                                  htmlBody
